...
public class GeneratorSourceCodeComponentImpl  extends GeneratorSourceCode{
	...
	public void generateTableListCode(DBRProgressMonitor monitor, StringBuilder sql, List<DBSTable> tablesOrViews,Map<String, Object> options) throws DBException {
	 ...
	 private String javaFileName(DBSTable table,String extFileName)
	 {
		 ...
		 String mClassFullName=getClassFullName(mTableName_upperCamelCase, getRuleComponentImpl());//类全称
...
//		 String lf = GeneralUtils.getDefaultLineSeparator();
     ...
     if (!CommonUtils.isEmpty(indexes)) {
		 ...
		 CodeHelper.addCodeLine(sql,String.format("package %s;", mComponentImpl_package) );
		 ...
		 CodeHelper.addCodeLine(sql, String.format("import org.springframework.beans.factory.annotation.Autowired;%simport org.springframework.stereotype.Component;",lf));
		 sql.append(lf);
		 if(!CodeHelper.isEmpty(pageClassFullName))
		 {
		 ...
		 CodeHelper.addCodeLine(sql, String.format("import %s;", mDao_fullName));
		 sql.append(lf);
		 ...
		 CodeHelper.addCodeLine(sql,String.format("@Component(\"%s\")",mComponent_paramName));
		 CodeHelper.addCodeLine(sql,String.format( "public class %s implements %s {",mComponentImpl_typeName,mComponent_typeName));
		 sql.append(lf);
		 
		 CodeHelper.addCodeLine(sql, String.format("@Autowired %sprivate %s %s;",lf, mDao_typeName,mDao_paramName));
		 sql.append(lf);
		 sql.append(lf);
		 
		 CodeHelper.addCodeLine(sql,String.format("@Override%spublic Integer insert%s(%s %s){%sreturn %s.insert(%s);%s}", lf,tableName_upperCamelCase,mEntity_typeName,mEntity_paramName
				 ,lf,mDao_paramName,mEntity_paramName,lf) );
		 sql.append(lf);
		 
		 CodeHelper.addCodeLine(sql,String.format("@Override%spublic List<%s> query%sList(Map<String,Object> selectItem){%sreturn %s.queryList(selectItem);%s}", lf,mEntity_typeName,tableName_upperCamelCase
				 ,lf,mDao_paramName,lf));
		 sql.append(lf);
		 
		 if(!CodeHelper.isEmpty(pageClassFullName))
		 {
			 
			 CodeHelper.addCodeLine(sql,String.format("@Override%spublic %s<%s> query%sListPage(Long pageIndex,Integer pageSize,Map<String,Object> selectItem){%s	if(selectItem==null){%s		selectItem=new HashMap<String,Object>();%s	}%s	%s<%s> pageBean=new %s<%s>(pageIndex, pageSize);%s	selectItem.put(\"page\", pageBean);%s	pageBean.setPage(%s.queryListPage(selectItem));%s 	return pageBean;%s}",
					 lf,pageClassSimpleName, mEntity_typeName,tableName_upperCamelCase
					 ,lf,lf,lf,lf,pageClassSimpleName,mEntity_typeName,pageClassSimpleName,mEntity_typeName,lf,lf,mDao_paramName,lf,lf));
			 sql.append(lf);
		 }
		 
		 if(!CodeHelper.isEmpty(primaryKey))
		 {
			 CodeHelper.addCodeLine(sql,String.format("@Override%spublic Integer update%sBy%s(%s %s){%s	return %s.updateBy%s(%s);%s}", lf,tableName_upperCamelCase,primaryKey_upperCamelCase,mEntity_typeName,mEntity_paramName
					 ,lf,mDao_paramName,primaryKey_upperCamelCase,mEntity_paramName,lf));
			 sql.append(lf);
			 
			 CodeHelper.addCodeLine(sql, String.format("@Override%spublic Integer delete%sBy%s(%s %s){%s	return %s.deleteBy%s(%s);%s}",lf,tableName_upperCamelCase,primaryKey_upperCamelCase,CodeHelper.columnType2JavaType(primaryTableColumn),primaryKey_lowerCamelCase
					 ,lf,mDao_paramName,primaryKey_upperCamelCase,primaryKey_lowerCamelCase,lf));
			 sql.append(lf);
			  
			 CodeHelper.addCodeLine(sql, String.format("@Override%spublic %s find%sBy%s(%s %s){%s	return %s.findBy%s(%s);%s}",lf, mEntity_typeName,tableName_upperCamelCase,primaryKey_upperCamelCase,CodeHelper.columnType2JavaType(primaryTableColumn),primaryKey_lowerCamelCase
					 ,lf,mDao_paramName,primaryKey_upperCamelCase,primaryKey_lowerCamelCase,lf));
			 sql.append(lf);
		 }
