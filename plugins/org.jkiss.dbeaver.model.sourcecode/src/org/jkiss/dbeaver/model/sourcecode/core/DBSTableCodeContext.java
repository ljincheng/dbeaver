...
public class DBSTableCodeContext extends EngineContext{
	...
	private CodeColumnAttribute mPrimaryKeyColumn;
	...
	public void pushColumnsAttribute(List<CodeColumnAttribute> columns,List<FormItemContext> settings)
	{
		if(columns!=null)
		{
			 ...
			 if(mPrimaryKeyColumn!=null) {
				 primaryKeyColumn= mPrimaryKeyColumn;
			 }
		        		 ...
		        		 for(DBSTableIndexColumn column:tableColumn)
		        		 {
		        			 ...
		        			 if(primaryColumn!=null)
		        			 {
			            			boolean useGeneratedKeys=primaryColumn.isAutoGenerated();
			            			 ...
			            			 mPrimaryKeyColumn=new CodeColumnAttribute();
			            			 mPrimaryKeyColumn.setColumnName(primaryKey);
			            			 mPrimaryKeyColumn.setDesciption(primaryColumn.getDescription());
			            			 mPrimaryKeyColumn.setIsAutoGenerated(useGeneratedKeys);
			            			 mPrimaryKeyColumn.setIsPrimary(true);
			            			 mPrimaryKeyColumn.setIsRequired(primaryColumn.isRequired());
			            			 mPrimaryKeyColumn.setCodeName(primaryKey_upperCamelCase);
			            			 mPrimaryKeyColumn.setParamName(primaryKey_lowerCamelCase);
			            			 mPrimaryKeyColumn.setJavaType(mPrimaryKey_typeName);
			            			 mPrimaryKeyColumn.setJavaPackage(getJavaFullType(mPrimaryKey_typeName));
			            			
			            			 dataMap.put(KEY_PRIMARYCOLUMN,mPrimaryKeyColumn);
		        			 }
		        		 }
			 ...
			 { 
				 ...
				 CodeColumnAttribute codeColumn=new CodeColumnAttribute();
				  ...
				  if(isAutoGenerated)
				  {
					 ...
					 if(mPrimaryKeyColumn!=null && columnName.equals(mPrimaryKeyColumn.getColumnName())){
						 codeColumn.setIsPrimary(true);
            			 codeColumn.setIsAutoGenerated(mPrimaryKeyColumn.getIsAutoGenerated());
					 }else {
